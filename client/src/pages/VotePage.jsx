...
>
  <input
    ...
    checked={
      (position === "President" &&
        presidentId === String(candidate.id)) ||
      (position === "Secretary" &&
        secretaryId === String(candidate.id)) ||
      (position === "Treasurer" &&
        treasurerId === String(candidate.id))
    }
    onChange={() => {
      if (position === "President")
        setPresidentId(String(candidate.id));

      if (position === "Secretary")
        setSecretaryId(String(candidate.id));

      if (position === "Treasurer")
        setTreasurerId(String(candidate.id));
    }}
  ...
  <div className="flex-1">
    ...
    <p className="text-sm text-slate-500">
      {candidate.details || "No additional details"}
    </p>
  </div>
